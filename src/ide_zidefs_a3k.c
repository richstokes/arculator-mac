...
typedef struct zidefs_a3k_ide_t
{
	int page;
	uint8_t rom[16384];
...
static int zidefs_a3k_ide_init(struct podule_t *podule)
{
	...
	resetide(&zidefs->ide,
		 ...
		 zidefs_a3k_ide_irq_raise, zidefs_a3k_ide_irq_clear);
...
static void zidefs_a3k_ide_reset(struct podule_t *podule)
{
	zidefs_a3k_ide_t *zidefs = podule->p;

	zidefs->page = 0;
}

static void zidefs_a3k_ide_close(struct podule_t *podule)
{
	zidefs_a3k_ide_t *zidefs = podule->p;
...
static uint8_t zidefs_a3k_ide_read_b(struct podule_t *podule, podule_io_type type, uint32_t addr)
{
	zidefs_a3k_ide_t *zidefs = podule->p;
	uint8_t temp = 0xff;
...
//                rpclog("Read IROM %04X %i %04X %02X\n",addr,icspage,temp,icsrom[temp&0x3FFF]);
		...
		if ((addr & 0x1c) == 0)
		{
			uint16_t tempw = readidew(&zidefs->ide);

			zidefs->high_byte_latch = tempw >> 8;
			temp = tempw & 0xff;
		}
		else
		...
		case 0x2800:
		temp = zidefs->high_byte_latch;
		break;
	}

//        rpclog("Read ZIDEFS %07X %02x %07X\n", addr, temp, PC);
	return temp;
}

static uint16_t zidefs_a3k_ide_read_w(struct podule_t *podule, podule_io_type type, uint32_t addr)
{
	return zidefs_a3k_ide_read_b(podule, type, addr);
}

static void zidefs_a3k_ide_write_b(struct podule_t *podule, podule_io_type type, uint32_t addr, uint8_t val)
{
	zidefs_a3k_ide_t *zidefs = podule->p;
...
//        rpclog("Write ZIDEFS %07X %02X %07X\n",addr,val,PC);
	switch (addr & 0x3c00)
	{
		case 0x2000:
		zidefs->page = val;
		...
		case 0x2400:
		if ((addr & 0x1c) == 0)
			writeidew(&zidefs->ide, val | (zidefs->high_byte_latch << 8));
		else
			writeide(&zidefs->ide, ((addr >> 2) & 7) + 0x1f0, val);
		break;

		case 0x2800:
		zidefs->high_byte_latch = val;
		break;
	}
}

static void zidefs_a3k_ide_write_w(struct podule_t *podule, podule_io_type type, uint32_t addr, uint16_t val)
{
	zidefs_a3k_ide_write_b(podule, type, addr, val & 0xff);
}

static const podule_header_t zidefs_a3k_ide_podule_header =
{
	.version = PODULE_API_VERSION,
	.flags = PODULE_FLAGS_UNIQUE | PODULE_FLAGS_8BIT,
	.short_name = "zidefs_a3k",
	.name = "ZIDEFS 8-bit IDE Controller",
	.functions =
	{
		.init = zidefs_a3k_ide_init,
		.close = zidefs_a3k_ide_close,
		.reset = zidefs_a3k_ide_reset,
		.read_b = zidefs_a3k_ide_read_b,
		.read_w = zidefs_a3k_ide_read_w,
		.write_b = zidefs_a3k_ide_write_b,
		.write_w = zidefs_a3k_ide_write_w
	},
	.config = &ide_podule_config
};

const podule_header_t *zidefs_a3k_ide_probe(const podule_callbacks_t *callbacks, char *path)
{
	FILE *f;
	char fn[512];
	...
	if (!f)
		return NULL;
	fclose(f);
	return &zidefs_a3k_ide_podule_header;
}
